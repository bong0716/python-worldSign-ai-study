...
# 데이터 로드
data = np.load('landmarks.npy')  # 저장된 랜드마크 데이터 로드
labels = np.load('labels.npy')  # 저장된 라벨 데이터 로드

# 데이터 형상 확인 및 변환
print(f"Original data shape: {data.shape}")  # 데이터의 원래 형상 출력

# 데이터를 (샘플 수, 21, 3) 형상으로 변환
...
# LSTM 모델 정의
model = Sequential()
model.add(LSTM(64, return_sequences=True, input_shape=(21, 3)))  # 첫 번째 LSTM 층, 64개의 유닛, 입력 모양 설정
...
# 모델 학습
model.fit(X_train, y_train, epochs=10, batch_size=32, validation_data=(X_test, y_test))  # 학습 데이터로 모델 학습, 10번의 에포크, 배치 크기 32

# 모델 저장
model.save('sign_language_model.h5')  # 학습된 모델 저장
